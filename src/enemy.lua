...
-- helper to randomly walk around without getting stuck
-- used by idle state
function Enemy:wander()
    -- get all possible directions we can move to
    local validDirs = {}
    local allDirs = {"north", "south", "east", "west"}

    for _, d in ipairs(allDirs) do
        if self:canGo(d) then
            table.insert(validDirs, d)
        end
    end
    ...
    -- if we are moving, try not to turn 180 degrees unless it's a dead end
    -- this makes movement look more natural
    if self.direction and #validDirs > 1 then
        local reverse = {north="south", south="north", east="west", west="east"}
        local backDir = reverse[self.direction]
        
        -- create a list without the back direction
        local forwardDirs = {}
        for _, d in ipairs(validDirs) do
            if d ~= backDir then
                table.insert(forwardDirs, d)
            end
        end

        if #forwardDirs > 0 then
            self.direction = forwardDirs[math.random(1, #forwardDirs)]
        else
            -- dead end, forced to turn back
            self.direction = backDir
        end
    else
        -- just pick random
        self.direction = validDirs[math.random(1, #validDirs)]
    end
end
